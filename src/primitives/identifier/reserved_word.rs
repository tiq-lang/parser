...
use crate::{
    byte_stream::core::ByteStream,
    error_stack::{ErrorStack, replace_with_uninit},
    primitives::identifier::{self, Identifier, IdentifierKind, IdentifierMeta},
};
use phf::phf_map;
use strum::EnumIter;
...
pub enum ParseResult {
    Ok(IdentifierOrReservedWord, IdentifierMeta),
    Panic,
}

/// Error type of `IdentifierOrReservedWord` parsing functions.
pub enum ParseError {
    IdentifierError(Box<identifier::ParseError>),
}
...
impl IdentifierOrReservedWord {
    ...
    /// `ParseResult::Panic` one `ParseError` is pushed onto error stack.
        ...
        E: ErrorStack + ?Sized,
    {
        match Identifier::parse(stream, errors)? {
            identifier::ParseResult::Ok(identifier, meta)
            ...
            identifier::ParseResult::Ok(identifier, meta) => {
                match RESERVED_WORDS.get(identifier.as_ref()) {
                    Some(&reserved_word) => Ok(ParseResult::Ok(
                        IdentifierOrReservedWord::ReservedWord(reserved_word),
                        meta,
                    )),
                    None => Ok(ParseResult::Ok(
                        IdentifierOrReservedWord::Identifier(identifier),
                        meta,
                    )),
                }
            }
            identifier::ParseResult::Panic => {
                let (old_error, new_error) = unsafe {
                    replace_with_uninit::<ParseError>(
                        errors
                            .inspect_last_error()
                            .expect("`Identifier::parse` should have produced an error"),
                    )
                };
                new_error.write(ParseError::IdentifierError(old_error.downcast().expect(
                    "`Identifier::parse` error should be of type `identifier::ParseError`",
                )));
...
mod tests {
    use super::*;
    use crate::{
        byte_stream::{core::Position, literal_byte_stream::LiteralByteStream},
        primitives::identifier::tests::CheckIdentifierArguments,
    };
    ...
    fn check_reserved_word(arguments: CheckReservedWordArguments) {
        ...
        match IdentifierOrReservedWord::parse(&mut stream, &mut errors).unwrap() {
            ParseResult::Ok(identifier_or_reserved_word, meta) => match identifier_or_reserved_word
            {
                IdentifierOrReservedWord::ReservedWord(parsed_reserved_word) => {
